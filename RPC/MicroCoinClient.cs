...
namespace MicroCoin.RPC {
	public class MicroCoinClient : IMicroCoinClient {
		...
		public MicroCoinClient() : this(MicroCoinClientConfiguration.DefaultMainNet) {			
		}

		public MicroCoinClient(MicroCoinClientConfiguration configuration) : this(configuration.Server, configuration.Port) {			
		}

		public MicroCoinClient(string server, int port = Constants.DefaultMainNetRpcPort) {
        ...
        public virtual AccountDTO[] FindMyAccounts(string pubkey, uint? start = null, uint? max = null)
        {
            return Invoke<AccountDTO[]>(ApiMethodName.findaccounts.ToString(), new Dictionary<string, object>()
            {
                ["enc_pubkey"] = pubkey,
		...
		protected virtual T Invoke<T>(string method, IDictionary<string, object> arguments = null) {
			using (var callScope = new CallScope(this)) {
				...
				webRequest.UserAgent = $"MicroCoin {Constants.NCoinVersion}";
				...
				try {
						...
						using (var responseStream = webResponse.GetResponseStream()) {
							using (var sr = new StreamReader(responseStream)) {
								...
								if (jsonResponse.Error != null) {
									throw new MicroCoinRPCException(JsonConvert.DeserializeObject<ErrorResultDTO>(jsonResponse.Error.ToString()));
								}
					...
					throw new MicroCoinRPCException(errorDTO);
				}
			}
		}
		...
		protected class CallScope : IDisposable {
			private readonly MicroCoinClient _client;
			public readonly int CallID;
			public CallScope(MicroCoinClient client) {
				_client = client;
